...
bam_partial_dependence(species = species_v5[1], bcr = bcraoi_v5[1], 
                       predictor = "SCANFIprcD_5x5", version = "v5", spar=0.8)

```

It looks like `SCANFIprcD_1km` (% broadleaf tree cover) is positively correlated with Canada Warbler abundance. Notice that we used `spar=0.8` to improve smoothness of the fit (see `?bam_partial_dependence` or `?smooth.spline` for details of how smoothness can be tuned by `spar`). We can repeat the same procedure for our other BCR of interest:
...
bam_partial_dependence(species = species_v5[1], bcr = bcraoi_v5[2], 
                       predictor = "SCANFIprcD_1km", version = "v5", spar=0.8)

```
The warbler ~ deciduous cover relationship is very similar in both BCRs.
