...
We'll use the same example as the previous vignette: species of conservation concern within an area of interest (AOI) across two sugbregions in the boreal plains and shield. This time, instead of accessing the models for the whole subregions, we'll use the `crop_ext` argument to access just the AOI.
...
Once we have our models for our AOI, we can use the `bam_occurrence` function to threshold the continuous density surface into a binary occurrence surface. 
...
```{r map_occu_lorenz, fig.width=8, fig.height=6, message = FALSE, warning= FALSE, echo = TRUE, results = 'markup', error = TRUE}
...
```{r map_occu_50, fig.width=8, fig.height=6, message = FALSE, warning= FALSE, echo = TRUE, results = 'markup', error = TRUE}
# Threshold the model with the lorenz curve threshold
...
occ_lorenz_result$occurrence_summary

# Area of occurence for quantile approach
occ_quantile_result$occurrence_summary
...
# Show results
kable(pop_aoi_result, caption = "Estimated population size")
```
...
Let's try grouping by bird conservation region (BCR; i.e.,our model subregions). First we'll need to create a new SpatVector with separate polygons for the two BCRs in our AOI.
...
We can see that now our total population is split approximately 40% in BCR 80 and 60% in BCR 61, primarily due to differences in mean density between the BCRs, and not by differences in total area.
