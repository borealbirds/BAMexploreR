...
The machine learning framework that BAM uses to produce the landbird density models provide an estimate of the percent importance of each predictor in the model and we can summarize those importances to understand the important habitat relationships for species of interest in each model subregion. Note that these functions will only provide the *importance* of each predictor, not the shape or direction of the relationship, which can be nonlinear and include complex interactions with other predictors. For more information on the BAM landbird density models, please see the 'Introduction' vignette.

## Functions available

### bam_covars_importance()

**bam_covars_importance()** plots mean relative predictor importance (with bootstrap variation) by predictor class, with options to group by BCR (Bird Conservation Region) or species.
...
**bam_covars_barchart()** creates stacked bar plots showing the proportion of model predictors importance in predicting bird abundance.
...
The `bam_covars_importance` function is a good way to start exploring predictor importance because it summarizes the predictor importance into nine general categories:
...
Now we're just looking at one species, Canada Warbler, but if we add another species and group by BCR, we get summarized statistics across species. Let's add some more species and see what we get.
...
# Create plot of predictor importance grouped by BCR
bam_covars_importance(species = spp_v5, bcr = bcraoi_v5, group = "bcr", version = "v5", plot=TRUE)

```

It looks like biomass is pretty important! There is a metadata table for the model predictors stored in the package. Let's check out what goes into the biomass class.
...
bam_covars_barchart(species = spp_v5, bcr = bcraoi_v5, groups=c("spp", "var_class"), version = "v5", plot=TRUE)
```

Again, we see that the biomass predictors have the greatest proportion of influence, but that it varies across species.

Let's look at the difference between model subregions, or BCRs.
...
bam_covars_barchart(species = spp_v5, bcr = bcraoi_v5, groups=c("bcr", "var_class"), version = "v5", plot=TRUE)
```
...
And now let's try looking at the proportion of influence of individual predictors by species.
...
bam_covars_barchart(species = spp_v5, bcr = bcraoi_v5, groups=c("var", "spp"), version = "v5", plot=TRUE)

```

We can see there's a lot of variation here. Not every predictor is included in every species model either. Let's try extracting the summary instead of the plot.

```{r cov_bar_text, echo = TRUE, results = 'markup', error = TRUE, fig.width=7, fig.height=4, fig.align='center'}
# Create plot of predictor importance for each species grouped by predictor class
bam_covars_barchart(species = spp_v5, bcr = bcraoi_v5, groups=c("var", "spp"), version = "v5", plot=FALSE)
