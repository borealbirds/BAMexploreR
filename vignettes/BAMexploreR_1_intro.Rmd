...
Welcome! `BAMexploreR` is a package for accessing and analyzing the current and archived Landbird Density & Habitat models produced by the [Boreal Avian Modelling Centre (BAM)](https://borealbirds.ca/).

This vignette provides a general introduction to the `BAMexploreR` package, including background on the BAM Landbird Density & Habitat models and their versions, an overview of the package purpose, core functionality, and the data included with the package. It highlights the types of data embedded in the package (e.g., shapefiles, species lists) and explains how they support the functions used to explore species occurrence and density across the BCRs' extent. This is intended as a starting point to familiarize users with the structure and content of the package before diving into specific workflows or analyses.
...
### Model versions
Two versions of the BAM Landbird Density & Habitat models are accessible through `BAMexploreR`: the archived models (Version 4) and the current models (Version 5).
...
```{r setup, eval=FALSE, include = TRUE, message=FALSE, warning=FALSE}
remotes::install_github('borealbirds/BAMexploreR')
library(BAMexploreR)
```
...
This reference table serves as a lookup of the species available for each version of the BAM Landbird Density & Habitat models using four-letter AOU code, common name, or scientific name.  The table also provides several ways of grouping species if you are interested in a particular suite of species including species of conservation concern, guild, and classifications from The State of Canada’s Birds report, which organizes species by broad biomes and groups that exhibit distinct and notable population trends.
...
This reference table can be used to understand the different predictors used in the BAM Landbird Density & Habitat models, including a description and source for each, as well as the classes they are grouped into for some of the habitat relationship functions.
...
bibentry(
  ...
  year = "2025",
  number = "16(10): e70405",
  doi = "https://doi.org/10.1002/ecs2.70405"
)
```
