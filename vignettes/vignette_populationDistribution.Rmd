...
The **occurrenceNM()** function estimates a species' presence/absence threshold based on a Lorenz curve, which summarizes the spatial inequality in predicted occurrence probabilities. This function helps distinguish between areas of high and low predicted use, by determining a threshold value that defines the most likely occupied areas.
...
The **pop_sizeNM()** function estimates the total population size by summarizing predicted densities of males across a spatial extent. This function supports cropping to a defined area (e.g., a region of interest) and group-based summaries if the extent includes a grouping variable (e.g., subregions, jurisdictions).
...
### Set  the area of interest (AOI) as the Canadian extent of BCRs 10 and 11 
...
# Load and extract Canada BCRs 10 and 11
bcr_v5 <- vect(system.file("extdata", "BAM_BCRNMv5_5072.shp", package = "BAMexploreR"))
aoi_sf <- bcr_v5[bcr_v5$subunit_ui %in% c("can10", "can11"), ]

plot(bcr_v5, main = "AOI defined by the Canadian extent of BCRs 10 and 11")
plot(aoi_sf, col = "red", add = TRUE)
 

```

### Mapping species occurence for TEWA and BBWO within the AOI
...
results <- getlayerNM(c("TEWA", "BBWO"), "v5", destfile = tempdir(), crop_ext = aoi_sf, year = "2010")
occ_results <- occurrenceNM(results, plot = TRUE)
...
occ_results <- pop_sizeNM(results, crop_ext = aoi_sf, group = "subunit_ui")
