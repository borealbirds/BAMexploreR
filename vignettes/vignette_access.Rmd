...
`BAMexploreR` is a package for downloading and analyzing boreal bird species data from the BAM National Model v4 and v5. Functions in the package perform the following tasks:
...
```{r overall_bcr_list, fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
...
bcraoi_v5 <- getBCR("v5", ext=aoi_sf) 
print(subUnit)

# Map extent on BCR subunit
map <- mapBCR("v5", ext=aoi_sf) + tmap::tm_layout(title = "BCR Mapping - Version 4", title.position = c("center", "top"))
suppressWarnings(print(map)) + tmap::tm_layout(title = "BCR Mapping - Version 4", title.position = c("center", "top"))
```
...
cosewic_spp <- sppList("v5", "COSEWIC_Status")
print(cosewic_spp)

# From the cosewic_spp list, download TEWA and VESP density maps for BCRs intersecting user AOI for the year 2010.
result <- getlayerNM(cosewic_spp, "v5", destfile = tempdir(), year = "2010", bcrNM=bcraoi_v5)
plot(result$TEWA)
```
