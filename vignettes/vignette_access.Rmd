...
Let's check the species group categories and definitions by accessing the `guild_opt` documentation.
...
# Generate a vector of Forest bird species codes available in version 5. 
...
# Generate a vector of Forest bird species codes available in version 5. 
cosewic_v5_cn <- bam_spp_list("v5", "commonName", "Forest_Birds")
cosewic_v5_cn
```

### 2. Identify BCR intersecting the area of interest
...
For our example, we'll use an example area of interest that intersects multiple model subregions in the boreal plains and shield for our model download.
...
Now that we have our species and area of interest, we can download our model products!
...
Let's down the model products for our list of COSEWIC species for the model subregions in our area of interest. To speed things up, we'll just pick two of our COSEWIC species.

```{r get_layer, fig.width=8, fig.height=6, echo = TRUE, results = 'markup', error = TRUE}

cosewic_v5_dl <- cosewic_v5[]
...
```{r plot_layer, fig.width=8, fig.height=6, echo = TRUE, results = 'markup', error = TRUE}
library(terra)
