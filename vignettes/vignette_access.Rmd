...
You can choose to access the predictions for the entire modelled area, just a single model subregion (version 5 only), or a custom AOI. Since model subregion names vary by version and country, the `bam_map_bcr` function ensures the correct naming is used. You can specify an extent and identify BCRs intersection the AOI.

For our example, we'll use an example AOI that intersects multiple model subregions in the boreal plains and shield for our model download. Let's get the list of model subregions that interest with our AOI.
...
And we can also visualize where our AOI fall on the map of all the model subregions for version 5.
...
Now that we have our species and AOI, we can download the model predictions!
...
Let's down the model products for our list of COSEWIC species for the model subregions in our AOI.
