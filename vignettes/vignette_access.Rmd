...
Due to the intersection of country and BCR as the modelling subregions in version 5 of the BAM landbird models, we use a country prefix and BCR number to name the model subregion. Note that BCR labels differ betwen version 4 and version 5 due to changes made to several of the BCRs.
...
**bam_map_bcr()** plots the model subregions for a chosen version and can optionally overlay a provided AOI SpatVector to highlight the area of interest.
...
guild_opt
```
In our example, we'll get four-letter AOU codes for 'COSEWIC' species available in version 5 of the models. You can also check the . 
...
### 2. Identify model subregions that intersect the area of interest

Next, we'll need to determine the area of interest for accessing the model products. 

You can choose to access the predictions for the entire modelled area, just a single model subregion (version 5 only), or a custom area of interest. Since model subregion names vary by version and country, the `bam_map_bcr` function ensures the correct naming is used. You can specify an extent and identify BCRs intersection the area of interest.

For our example, we'll use an example area of interest that intersects multiple model subregions in the boreal plains and shield for our model download. Let's get the list of model subregions that interest with our area of interest.
...
And we can also visualize where our area of interest fall on the map of all the model subregions for version 5.

```{r aoi_bcr_map, echo = TRUE, results = 'markup', error = TRUE, fig.width=8, fig.height=6}
# Map aoi on the map of model subregions
map <- bam_map_bcr("v5", ext=aoi_sf) +
  tmap::tm_layout(title = "Model Subregions - Version 5", title.position = c("center", "top"))
...
Now that we have our species and area of interest, we can download the model predictions!

We will download density maps for species using the `bam_get_layer` function. In version 5, we also have the option of downloading multiple years of predictions, as the density models are available for five-year intervals from 1990 to 2020 (default is 2020). 

Let's down the model products for our list of COSEWIC species for the model subregions in our area of interest.
...
- **truncated_mean**: the mean predicted density in males/ha across 32 bootstraps, truncated to the probable range of the species
...
We can plot these bands for one of our species of interest! Let's try Canada Warbler.

```{r plot_layer, echo = TRUE, results = 'markup', error = TRUE, fig.width=8, fig.height=6}
library(terra)
