...
result_sub <- list("CAWA" = result[["CAWA"]], "OSFL" = result[["OSFL"]])
occ_lorenz_result <- bam_occurrence(result_sub, plot = TRUE)
```
...
occ_quantile_result <- bam_occurrence(result_sub, quantile = 0.5, plot = TRUE)
```
...
result_sub <- list("CAWA" = result[["CAWA"]], "OSFL" = result[["OSFL"]])
pop_aoi_result <- bam_pop_size(result_sub)
...
We can see that our total Olive-sided Flycatcher population is primarily in BCR 80. We can see based on the mean_density and n_cells columns that this larger population size is primarily due to differences in mean density between the BCRs, and not by differences in total area suggesting BCR 61, at least within our AOI provides higher quality habitat for Olive-sided Flycatchers.
