...
This vignette shows you how to use the BAM Landbird Density & Habitat models to evaluate distribution and population size with the `BAMexploreR` package.

## Background

### Density estimates

The BAM Landbird Density & Habitat models use the [QPAD analytical approach for data integration](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210x.12106), which uses statistical offsets to account for imperfect detection and produce estimates of population density. The model products are species-specific predictions of the density of breeding male birds per hectare at 1 km resolution across their modelled extents. These density surfaces can support a variety of conservation and land-use planning applications. The `BAMexploreR` package provides functions for two core analytical tasks. For more information on the models, see the 'Introduction' vignette.

## Functions available

### bam_occurrence()

**bam_occurrence()** estimates a species' presence/absence threshold based on a Lorenz curve. This curve is fitted using the [opticut package](https://rdrr.io/cran/opticut/man/lorenz.html), which analyses the spatial inequality in predicted density across an area of interest. `opticut` distinguishes "presence" from "absence" by identifying pixels of a certain density value that have a negligible contribution to the area's total population. Therefore, this function helps distinguish between areas of high and low predicted use, by determining a threshold value that defines the most likely occupied areas.
...
We'll use the same example as the previous vignette: Olive-sided Flycatcher within an area of interest (AOI) across two sugbregions in the boreal plains and shield. This time, instead of downloading the model products for the whole subregions, we'll use the `crop_ext` argument to download just the AOI.
...
result <- bam_get_layer("OSFL", "v5", destfolder = tempdir(), year = "2020", crop_ext=aoi_sf)
```

Let's plot just the mean density to check our download.

```{r plot_layer, echo = TRUE, results = 'markup', error = TRUE, fig.width=7, fig.height=5}
library(terra)
plot(result$OSFL$mean)
...
Let's try the default option, `by_lorenz` first on downloaded models for Olive-sided Flycatcher.
...
knitr::kable(occ_lorenz_result$occurrence_summary, caption="Area of occurence with Lorenz threshold")
# Area of occurence for quantile approach
knitr::kable(occ_quantile_result$occurrence_summary, caption="Area of occurence with 50% threshold")
```

Within our AOI of ~100,000 km^2^, we estimate approximately half of that is occupied by Olive-sided Flycatchers if we use the Lorenz curve approach and we estimate just under a third of that is occupied by Olive-sided Flycatchers if we use a custom 50% threshold
...
Let's use our same Olive-sided Flycatcher example as before. 
...
# Show results
knitr::kable(pop_aoi_result, caption = "Estimated population size (male birds)")
```
...
Let's imagine a regional planning project where we need to know which BCR (in our AOI) has a larger Olive-sided Flycatcher population. We might start by creating a new SpatVector with separate polygons for the two BCRs in our AOI.


```{r aoi_bcr_map, echo = TRUE, results = 'markup', error = TRUE, fig.width=7, fig.height=4}
# Get the BCR names that intersect our AOI
bcraoi_v5 <- bam_get_bcr(version="v5", ext=aoi_sf) 

# Get the BCRs of interest from model subregion layer provided in the package
bcr_v5 <- vect(system.file("extdata", "BAM_BCRNMv5_3978.shp", package = "BAMexploreR"))
aoi_bcr <- bcr_v5[bcr_v5$bcr %in% bcraoi_v5, ]

# Crop the BCR SpatVector to our AOI
aoi_sf_bcr <- terra::crop(aoi_bcr, aoi_sf)
...
# Calculate population size based on BAM landbird density raster
pop_bcr_result <- bam_pop_size(result, crop_ext=aoi_sf_bcr, group="bcr")

# Show results
knitr::kable(pop_bcr_result, caption = "Estimated population size")
```

We can see that our total Olive-sided Flycatcher population is primarily in BCR 80. Based on the mean_density and n_cells columns that this larger population size is primarily due to differences in mean density between the BCRs, and not by differences in total area suggesting BCR 61 provides higher quality habitat for Olive-sided Flycatchers (within our AOI).
